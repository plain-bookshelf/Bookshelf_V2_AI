...
from app.v1.domain.chat.services.llm.memory.messages import get_messages
...
class SetPrompt:
    ...
    def sim_query_prompt(session: Session, con_id) -> str:
        ...
        instructions = (
            ...
            "그 외 이전의 정보를 암시하는 등의 질문 등 필요 없는 경우 'False'를 반환하라\n"
            "예시: 그 책 청구기호 뭐야? = False, 그 책들 중 더욱 추천할만한 거 있어? = False, 그 책과 비슷한 책 추천해줘 = True\n"
    ...
    @staticmethod
    def rewrite_query(session: Session, con_id: int, book_datas: str = "") -> str:
        recent_chat = get_messages(session, con_id)
        instructions = (
            "너는 검색 쿼리 재작성 기계다.\n"
            "사용자의 질문이 이전 대화를 참조하는 경우(예: '그 책', '비슷한 책', '그중에서'),\n"
            "대화 맥락을 반영해 혼자서도 의미가 완전한 검색 쿼리로 재작성해라.\n"
            "이전 대화를 참조하지 않는 독립적인 질문이면 그대로 출력해라.\n"
            "검색 쿼리 텍스트만 출력해라. 설명, 따옴표, 기타 텍스트 금지.\n"
            "────────────────\n"
            f"[이전 대화]\n{recent_chat}\n"
            f"[이전에 추천된 책]\n{book_datas if book_datas else 'none'}\n"
            "────────────────\n"
            "예시:\n"
            "질문: '그럼 그 책과 비슷한 책 또 없어?'\n"
            "이전 대화에서 해리포터를 추천했다면 →  '해리포터처럼 마법과 모험이 주제인 판타지 소설'\n"
        )
        return instructions


    @staticmethod
    def book_prompt(session: Session, chunk, con_id) -> str:
        user_memory_summary = memory_summary(session, con_id)
        recent_chat = get_messages(session, con_id)
        book_datas = get_book_datas(session, con_id)
        ...
        instruction = f"""
            ...
            [ACTIVE_BOOK_META]
            - 이 블록은 최근 대화에서 추천했거나 논의 중인 '현재 활성화된 책'들의 핵심 정보이다.
            - 사용자가 "그 책 청구기호 알려줘"처럼 지시대명사를 사용해 물어보면 이 정보를 보고 대답해라.
            {book_datas}
